...
async function loadProducts() {
    ...
    for (var i = 0; i < products.length; i++) {
        var p = products[i];
        html += '<div onclick="window.handleAddToCart(' + p.id + ')" class="bg-slate-800 p-4 rounded-xl border border-slate-700 hover:border-amber-500 cursor-pointer transition-all hover:shadow-lg hover:shadow-amber-500/10 active:scale-95">' +
...
function renderCartUI() {
    ...
    if (cart.length === 0) {
        ...
        for (var i = 0; i < cart.length; i++) {
            var item = cart[i];
            html += '<div class="flex items-center justify-between py-3 border-b border-slate-700">' +
                '<div class="flex-1"><p class="text-white font-semibold text-sm">' + item.name + '</p><p class="text-slate-400 text-xs">' + item.qty + ' x Rp ' + item.price.toLocaleString('id-ID') + '</p></div>' +
                '<div class="flex items-center gap-3"><span class="text-white font-bold text-sm">Rp ' + (item.qty * item.price).toLocaleString('id-ID') + '</span><button onclick="window.handleRemoveFromCart(' + item.id + ')" class="text-red-400 hover:text-red-300"><i class="fas fa-times"></i></button></div>' +
                '</div>';
        }
